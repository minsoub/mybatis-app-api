package kr.co.fns.app.core.openGraph;
...
//        HttpURLConnection httpURLConnection = (HttpURLConnection) siteConnection;
//        Charset charset = getConnectionCharset(httpURLConnection);
//        BufferedReader dis = new BufferedReader(new InputStreamReader(httpURLConnection.getInputStream(), charset));
//        String inputLine;
...
//        }
        ...
        final Elements elements = document.select("head");

        String headContentsStr = elements.toString();
        ...
        // og:title 값이 없을경우 <title> 태그의 값을 넣어주기 위해 title 값을 저장한다.
        TagNode[] titleData = pageData.getElementsByName("title", true);
        for(TagNode titleElement : titleData) {
            title = title.concat(titleElement.getText().toString());
        }
        ...
        if (!ignoreSpecErrors) {
            for (String req : REQUIRED_META)
            {
                if (!metaAttributes.containsKey(req))
                    throw new FantooException(ErrorCode.ERROR_FE2012);
            }
        }

        // TODO type check 필요하면 다시 사용할 수 있음.
        /**
